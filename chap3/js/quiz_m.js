var dap1=2;
var dap2=["소프트웨어업데이트","취약점","비활성화"];
var cstr_array=["ㅅㅍㅌㅇㅇㅇㄷㅇㅌ","ㅊㅇㅈ","ㅂㅎㅅㅎ"]
var quiz_tot=2;
var munNum=1;
var audio=null;
...
$(function(){
    ...
    setPlaceHolder_fn($(".Sentence_1 input"),cstr_array[0]);
    setPlaceHolder_fn($(".Sentence_2 input"),cstr_array[1]);
    setPlaceHolder_fn($(".Sentence_3 input"),cstr_array[2]);
    ...
    function chkDap2_fn(){//2번 문제 정답 확인
        var rtn_bool=true;
        rtn_bool = rtn_bool && chkSentences_fn($(".Sentence_1 input"), dap2[0]);
        rtn_bool = rtn_bool && chkSentences_fn($(".Sentence_2 input"), dap2[1]);
        rtn_bool = rtn_bool && chkSentences_fn($(".Sentence_3 input"), dap2[2]);
      // 정답 여부에 따라 사운드 재생
      if (rtn_bool) {
    ...
    function chkSentences_fn(inputs, d_str) {
        var match_bool = true; // 모든 입력이 정답이어야 하므로 기본값 true
        for (var i = 0; i < d_str.length; i++) {
            if (inputs.eq(i).val() != d_str.charAt(i)) {
                match_bool = false; // 틀린 입력이 있으면 false
                inputs.eq(i).val(""); // 틀린 입력 초기화
            }
        }
        return match_bool;
    }
    ...
    function fillDap2_fn(){
        inputSentences_fn($(".Sentence_1 input"),dap2[0]);
        inputSentences_fn($(".Sentence_2 input"),dap2[1]);
        inputSentences_fn($(".Sentence_3 input"),dap2[2]);
